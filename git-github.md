...
Returns to earlier specified (by its Hash code) commit
Returns to the specified commit and unstage all changes made after
``` git reset <commitHash> ```
Returns to the specified commit and stages (adds) all changes made after
``` git reset --soft <commitHash> ```
Returns to the specified commit and unstage all changes made after (default option - the same as git reset)
``` git reset --mixed <commitHash> ```
Returns to the specified commit and discards all changes made after
...
w => write
q => exit
*Depending on the model, version and configuration of your git and IDE, if your Git is vinculated to a IDE it opens the commit in a git commit file in your IDE, this file can be normally edited and changes you take effect when closed.
